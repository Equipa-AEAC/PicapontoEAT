...
class SpaHandler extends AbstractHandler {
    ...
    public function execute(request:Request):AbstractResponse {
        ...
        if (!request.path.startsWith("/api")) {
            var indexPath:String = Path.normalize('${publicPath}/index.html');
            if (FileSystem.exists(indexPath) && !FileSystem.isDirectory(indexPath)) {
                var fileInput = File.read(indexPath);
                try {
                    var fileInputResponse:FileInputResponse = new FileInputResponse(fileInput);
                    fileInputResponse.header("Content-Type", "text/html");
                    return fileInputResponse;
                } catch (e) {
                    fileInput.close();
                    throw e;
